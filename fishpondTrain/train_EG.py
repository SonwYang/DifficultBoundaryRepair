...
from models.EGNet import EGNet
...
class Trainer(object):
    def __init__(self, cfg):
        ...
        self.model = EGNet(num_classes=3, in_channels=8, encoder_depth=34).to(self.device).apply(weight_init)
    ...
    def validation(self, epoch, dataloader):
        ...
        for batch_id, sample_batched in enumerate(dataloader):
            ...
            wm = sample_batched['wm'].to(self.device)
            file_name = sample_batched['file_name']

            segments, _ = self.model(images)
    ...
    def train(self):
        ...
        for epoch in range(1, self.cfg.num_epochs):
            ...
            for batch_id, sample_batched in enumerate(train_loader):
                ...
                images = sample_batched['image'].to(self.device)  # BxCxHxW
                labels_seg = sample_batched['gt'].to(self.device)  # BxHxW
                labels_edge = sample_batched['edge'].to(self.device)  # BxHxW
                wm = sample_batched['wm'].to(self.device)

                segments, edges = self.model(images)

                loss_seg = self.criterion_seg(segments, labels_seg, wm)
                loss_edge = sum([self.criterion_edge(center, labels_edge, l_weight=l_w) for center, l_w
