...
class WeightedFocalLoss2d(nn.Module):
    ...
    def forward(self, input, target, weight=None):
        ...
        ## soft label
        ymask = torch.where(ymask == 1., 0.95, 0.05)
...
def bdcn_lossV2(inputs, targets, wm=None, l_weight=1.1):
    ...
    try:
        ...
        print(f'the max value of targets: {torch.max(targets)}')
...
def WeightCrossEntropyLoss(inputs, targets):
    mask = (targets < 36).float()
    b, h, w = mask.shape
    pos = torch.sum(mask, dim=[1, 2], keepdim=True).float()
    weight = torch.zeros_like(mask)  # Shape: [b,].
    neg = h * w - pos
    beta = neg * 1. / (pos + neg)
    weight.masked_scatter_(targets > 0.,
    ...
    weight.masked_scatter_(targets <= 0.,
                           torch.ones_like(targets) * (11 * (1 - beta)))
    nll_loss = nn.NLLLoss(None, True, 255, None, reduction='none')
    log_p = F.log_softmax(inputs, dim=1)
    loss = nll_loss(log_p, targets)
    return loss
